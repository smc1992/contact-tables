  ...
  try {
    ...
    if (!rateLimitPassed) {
      ...
      if (res.statusCode === 202) {
        ...
        // Persist all recipients as pending so scheduled processor can pick them up
        try {
          ...
          if (recipientRows.length > 0) {
            ...
            for (let i = 0; i < recipientRows.length; i += chunkSize) {
              ...
              await adminSupabase.from('email_recipients').insert(chunk);
            }
          }
        } catch (e) {
          console.error('Failed to persist recipients for scheduled campaign:', e);
        }
      }
